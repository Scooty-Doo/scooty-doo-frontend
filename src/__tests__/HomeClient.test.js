...
jest.mock('../components/Map.js', () => () => <div>Mocked MapView</div>);
...
describe('HomeClient', () => {
  ...
  test('renderar komponenten korrekt', () => {
    render(
      <Router>
        <HomeClient />
      </Router>
  ...
  test('start a trip', async () => {
    ...
    await waitFor(() => {
      expect(startRide).toHaveBeenCalledWith('652134919185249719', '2');
    });
  ...
  test('end a trip', async () => {
    ...
    await waitFor(() => {
      expect(endRide).toHaveBeenCalledWith('mockTripId', '652134919185249719', '2');
    });
